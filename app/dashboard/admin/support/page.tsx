...
async function updateSupportRequestStatus(formData: FormData) {
  ...
  await sb.from("support_requests").update({ status }).eq("id", id);
...
if (activeAssigned === "unassigned") {
  ...
  const requests = ((data ?? []) as SupportRequest[]).filter(
  (request) =>
    !normalizedSearch ||
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-neutral-600">
              Review, prioritize, assign, and manage support requests submitted through Ko-Host.
            </p>
          </div>

<div className="flex flex-wrap gap-2">
  <a
    href="/api/dashboard/admin/export/support"
...
<form method="GET" className="mt-4">
  <div className="flex flex-col gap-2 sm:flex-row">
    <input
      name="search"
      defaultValue={search}
      placeholder="Search by name, email, message, category, or assignee..."
