...
export default function TournamentDisplayBlock({ block }: Props) {
  ...
  const baseMatches = Array.isArray(data.matches) ? data.matches : [];
  const shouldAutoGenerateBracket = Boolean((data as any).autoGenerateBracket ?? true);
  const matches =
    shouldAutoGenerateBracket && (data.displayMode ?? "bracket") === "bracket"
      ? buildAutoGeneratedBracketMatches(teams, baseMatches)
      : baseMatches;
