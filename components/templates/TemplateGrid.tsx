...
import { TEMPLATE_DEFS, type TemplateDef, type TemplateKey, type TemplateCategory } from "@/lib/templates/registry";
...
function metaFromRegistry(t: TemplateDef): PreviewMeta {
  return {
    tags: t.tags ?? [],
...
export default function TemplateGrid(props: {
  ...
  function openPreview(templateKey: string) {
    const t = allTemplates.find((x) => x.key === (templateKey as TemplateKey));
    if (!t) return;

    setPreviewTemplate(t);
    setPreviewThumb(thumbToImageUrl(t.thumb));
    setPreviewMeta(metaFromRegistry(t));
    setPreviewOpen(true);
  }
  ...
  const filteredTemplates = useMemo(() => {
    ...
    const badgeRank = (b: TemplateDef["badge"], target: "New" | "Popular") =>
      b === target ? 0 : b === null || b === undefined ? 2 : 1;
    ...
    if (sort === "New") {
      sorted.sort((a, b) => {
        const ra = badgeRank(a.badge, "New");
        const rb = badgeRank(b.badge, "New");
    ...
    if (sort === "Popular") {
      sorted.sort((a, b) => {
        const ra = badgeRank(a.badge, "Popular");
        const rb = badgeRank(b.badge, "Popular");
        if (ra !== rb) return ra - rb;
        return byTitle(a, b);
      });
      return sorted;
