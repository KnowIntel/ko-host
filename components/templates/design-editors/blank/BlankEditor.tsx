...
type CanvasItemWithLayer = CanvasGridItem & {
  grid?: Partial<GridPlacementWithLayer>;
};
...
function normalizeCanvasItems(items: CanvasItemWithLayer[]) {
  return items.map((item, index) => ({
    ...item,
    grid: normalizeGrid(item.grid, index + 1),
  }));
}

function moveCanvasItemToCell(
  items: CanvasItemWithLayer[],
  blockId: string,
  ...
  return normalizeCanvasItems(
    items.map((item) => {
      ...
      const grid = normalizeGrid(item.grid, 1);
...
function bringCanvasItemToFront(items: CanvasItemWithLayer[], blockId: string) {
  const highest = items.reduce((max, item, index) => {
    const z = normalizeGrid(item.grid, index + 1).zIndex || 1;
  ...
  return normalizeCanvasItems(
    items.map((item, index) => {
      ...
      const grid = normalizeGrid(item.grid, index + 1);
      return {
        ...item,
        grid: {
          ...grid,
...
function getDefaultPageElementGrid(
  ...
  if (key === "title") {
    return { colStart: 1, rowStart: 1, colSpan: 12, rowSpan: 1.5, zIndex: 1 };
  }

  if (key === "subtitle") {
    return { colStart: 1, rowStart: 2.5, colSpan: 10, rowSpan: 1, zIndex: 2 };
  }

  if (key === "subtext") {
    return { colStart: 1, rowStart: 3.5, colSpan: 8, rowSpan: 1, zIndex: 3 };
  }

  return { colStart: 1, rowStart: 4.5, colSpan: 9, rowSpan: 1.5, zIndex: 4 };
}

function buildPageCanvasItems(draft: BuilderDraft): CanvasItemWithLayer[] {
  const visibility = getPageVisibility(draft);
  const pageElements = coerceDraft(draft).pageElements ?? {};
  const items: CanvasItemWithLayer[] = [];
...
function toLayeredGrid(
  grid: GridPlacement | undefined,
  fallbackIndex: number,
): GridPlacementWithLayer {
  return normalizeGrid(
    grid as Partial<GridPlacementWithLayer> | undefined,
    fallbackIndex,
  );
}
...
function applyCanvasItemsToDraft(
  prev: BuilderDraft,
  canvasItems: CanvasItemWithLayer[],
): BuilderDraft {
  ...
  const blockGridMap = new Map<string, GridPlacementWithLayer>(
    canvasItems
      ...
      .map((item, index) => [item.id, normalizeGrid(item.grid, index + 10)]),
  );
  ...
  return {
    ...
    blocks: prev.blocks.map((block, index) => ({
      ...block,
      grid: (blockGridMap.get(block.id) ??
        toLayeredGrid(block.grid, index + 10)) as GridPlacement,
    })),
  };
}
...
export default function BlankEditor({ draft, setDraft }: Props) {
  ...
  const layeredBlocks = useMemo(() => {
    return draft.blocks.map((block, index) => ({
      ...block,
      grid: toLayeredGrid(block.grid, index + 10) as GridPlacement,
    }));
  }, [draft.blocks]);

  const canvasItems = useMemo(() => {
    const blockItems: CanvasItemWithLayer[] = layeredBlocks.map(
      (block, index) => ({
        id: block.id,
        type: block.type,
        label: block.label,
    ...
    return normalizeCanvasItems([...pageCanvasItems, ...blockItems]);
  }, [layeredBlocks, pageCanvasItems]);

  const ctaBlock = useMemo(
    () => getHeroButtonBlock(layeredBlocks) as CtaBlock | null,
    [layeredBlocks],
  );

  const countdownBlock = useMemo(
    () => getCountdownBlock(layeredBlocks) as CountdownBlock | null,
    [layeredBlocks],
  );

  const linksBlock = useMemo(
    () => getLinksBlock(layeredBlocks) as LinksBlock | null,
    [layeredBlocks],
  );

  const labelBlocks = useMemo(
    () => getLabelBlocks(layeredBlocks) as LabelBlock[],
    [layeredBlocks],
  );

  const imageBlocks = useMemo(
    () => getImageBlocks(layeredBlocks) as ImageBlock[],
    [layeredBlocks],
  );
        ...
        bringCanvasItemToFront(
          [
            ...
            ...prev.blocks.map((block, index) => ({
              ...
              type: block.type,
              label: block.label,
              grid: toLayeredGrid(block.grid, index + 10),
            })),
          ],
          blockId,
        ),
  ...
  function renderCanvasPreview(item: CanvasGridItem) {
      ...
      return (
        <div className="h-full">
          ...
          <div
            ...
            style={getTextStyle(draft.titleStyle)}
          >
    ...
    if (item.id === PAGE_DESCRIPTION_BLOCK_ID) {
      return (
        <div className="h-full">
          ...
          <div
            className="mt-3 text-neutral-700"
            style={getTextStyle(draft.descriptionStyle)}
          >
    ...
    const block = layeredBlocks.find((b) => b.id === item.id);
    ...
    switch (block.type) {
      case "label":
        return (
          <div
            className="flex h-full items-center justify-center rounded-xl border border-neutral-200 bg-white px-4 text-center text-neutral-900"
      ...
      case "cta":
        return (
          <div className="flex h-full flex-col justify-center">
            <div className="inline-flex w-fit rounded-xl bg-neutral-900 px-4 py-2 text-sm font-semibold text-white">
      ...
      case "links":
        return (
          <div className="grid h-full grid-cols-3 gap-2">
            {block.data.items.slice(0, 3).map((entry: LinkItem) => (
              <div
                key={entry.id}
                className="rounded-lg border border-neutral-200 bg-white px-2 py-3 text-center text-xs text-neutral-700"
              >
                {entry.label || "Link"}
              </div>
            ))}
      ...
      case "countdown":
        return (
          <div className="flex h-full flex-col justify-center rounded-xl border border-neutral-200 bg-white px-4 text-neutral-900">
      ...
      default:
        return <div className="text-xs uppercase text-neutral-500">{block.type}</div>;
    }
  }
            ...
            <div
              ...
              style={{
                ...
                backgroundImage: canvasBackgroundImage
                  ? `linear-gradient(rgba(255,255,255,0.08), rgba(255,255,255,0.08)), url(${canvasBackgroundImage})`
                  : undefined,
                backgroundSize: "cover",
                backgroundPosition: "center",
            ...
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() =>
                    bringCanvasBlockToFront(PAGE_SUBTITLE_BLOCK_ID)
                  }
                  className={panelButtonClass()}
                >
                  Bring to front
            ...
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() =>
                    bringCanvasBlockToFront(PAGE_DESCRIPTION_BLOCK_ID)
                  }
                  className={panelButtonClass()}
                >
                  Bring to front
                  ...
                  {block.data.options.map((option: PollOption) => (
                    <div key={option.id} className="flex gap-2">
                      <input
                        ...
                        className={`flex-1 ${panelFieldClass()}`}
                      />
                      <button
                        type="button"
                        onClick={() =>
